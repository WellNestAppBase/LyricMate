    ...
    <div className="w-full max-w-[435px] min-h-[600px] h-full sm:h-[932px] relative bg-[#0C121F] rounded-2xl overflow-hidden mx-auto">
      ...
      <div className="p-4 sm:p-7 flex justify-between items-center">
        ...
        <div className="relative">
          ...
          <div className="absolute -top-1 -right-1 w-3 h-3 bg-[#F38BDC] rounded-full"></div>
        </div>
      </div>
      ...
      <div className="px-4 sm:px-7 mb-4 sm:mb-6">
        <div className="relative">
          <div className="flex items-center gap-2 bg-[#1e1e1e] rounded-full pr-4">
      ...
      {/* Album list */}
      <div className="px-4 sm:px-7 space-y-4 sm:space-y-6 overflow-y-auto pb-24 max-h-[calc(100%-220px)]">
        {filteredAlbums.length > 0 ? (
          filteredAlbums.map((album) => (
            <div key={album.id} className="relative rounded-xl overflow-hidden">
              <img
                src={album.image}
                alt={album.title}
              ...
              <div className="absolute bottom-0 left-0 p-4 text-white">
                <div className="text-xl font-medium">{album.tracks} TRACKS</div>
                <div className="text-base font-medium text-white/70">
                  {album.likes} likes
                </div>
              </div>
              <button
                className="absolute bottom-4 right-4 bg-[#F38BDC] rounded-full p-2 transition-transform hover:scale-105 active:scale-95"
      ...
      <div className="fixed bottom-24 left-0 right-0 flex justify-center gap-4 z-10 px-4 sm:px-7 max-w-[435px] mx-auto">
        <button
          className="bg-[#F38BDC] text-white rounded-full p-4 shadow-neon-glow flex flex-col items-center transition-transform hover:scale-105 active:scale-95"
          onClick={() =>
            alert("Sing to identify songs by singing a melody or lyrics")
          }
        >
          <Mic className="h-6 w-6" />
          <span className="text-xs mt-1 font-medium">Sing</span>
        </button>
        <button
          className="bg-[#F38BDC] text-white rounded-full p-4 shadow-neon-glow flex flex-col items-center transition-transform hover:scale-105 active:scale-95"
        ...
        >
          ...
          <span className="text-xs mt-1 font-medium">Listen</span>
        </button>
      </div>
